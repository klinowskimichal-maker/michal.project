...
function renderMarketCoverage(){
  ...
  panel.innerHTML=`<section class="portal-more" aria-label="Dostępność ofert w portalach"><h3>Oferty bezpośrednio w portalach</h3><p>Kraj filtruje zapisane oferty PSKŁ. W portalach zagranicznych wyszukiwanie obejmuje tamtejszy rynek — nazwa wybranego kraju nie jest dopisywana do frazy. Określenie materiału jest tłumaczone na język portalu; nazwa marki lub modelu pozostaje bez zmian.</p><p>Przy braku odczytanych ofert automatyczna aktualizacja próbuje także określeń mahoniu w języku portalu, o ile portal pozwala na odczyt. Wyszukiwanie mahoniowych łodzi można też otworzyć osobnym przyciskiem. Przyciski otwierają wyszukiwanie w wybranym serwisie. Telefon może otworzyć jego zainstalowaną aplikację, jeśli obsługuje te linki; w pozostałych przypadkach otworzy stronę portalu.</p><div class="market-source-grid">${groups.map(group=>{
    const p=group[0],health=window.PSKL_MARKET.sourceStatus(p.name,status);
    return `<article class="market-source" data-source="${esc(p.id)}" data-state="${health.state}"><h4>${esc(p.name)}</h4><p class="market-source-scope">Rynek: ${esc(p.country)}${p.foreign?` · poza wybranym krajem (${esc(c.country)})`:''}</p><strong class="market-source-state">${label[health.state]}</strong><p>${esc(health.text)}</p><a class="button primary" href="${esc(p.href)}" target="_blank" rel="noopener">Otwórz ${esc(p.name)} ↗</a><small>Zapytanie (${esc(p.language)}): ${esc(p.query||'wszystkie łodzie')}</small>${group.filter(v=>!v.mahogany).length>1?`<div class="market-source-variants"><span>Sprawdź też:</span>${group.filter(v=>!v.mahogany).slice(1).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}${group.some(v=>v.mahogany)?`<div class="market-source-variants"><span>Brak wyników? Szukaj mahoniowych:</span>${group.filter(v=>v.mahogany).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}</article>`;
