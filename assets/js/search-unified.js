...
function renderMarketCoverage(){
  ...
  const label=EN?{loading:'Checking',blocked:'Automated retrieval blocked',unavailable:'Retrieval unavailable',unknown:'Retrieval not confirmed',unconfirmed:'Material not confirmed',empty:'No listings retrieved',partial:'Partial retrieval'}:{loading:'Sprawdzanie',blocked:'Pobieranie zablokowane',unavailable:'Odczyt niedostępny',unknown:'Odczyt niepotwierdzony',unconfirmed:'Materiał niepotwierdzony',empty:'Brak odczytanych ofert',partial:'Częściowy odczyt'};
  const healthEN={loading:'Checking the latest update...',blocked:'This marketplace blocks automated retrieval of listings into PSKL. Open the search directly on the marketplace.',unavailable:'Some or all listings could not be retrieved. Open the search on the marketplace.',unknown:'No confirmed retrieval of listings from this marketplace.',unconfirmed:'Listing cards were retrieved, but their content does not confirm a wooden hull. Check the material in the original listings.',empty:'The latest update did not retrieve any listings. This does not mean that the marketplace has no listings.',partial:'Some listings were retrieved. The PSKL index does not cover every listing on this marketplace.'};
  const intro1=EN?'Country filters the saved PSKL listings. Searches on foreign marketplaces cover their own markets, so the selected country is not added to the search phrase. The material is translated into the marketplace language, while the make or model remains unchanged.':'Kraj filtruje zapisane oferty PSKL. W portalach zagranicznych wyszukiwanie obejmuje tamtejszy rynek — nazwa wybranego kraju nie jest dopisywana do frazy. Określenie materiału jest tłumaczone na język portalu; nazwa marki lub modelu pozostaje bez zmian.';
  const intro2=EN?'If no listings are retrieved, the automatic update also tries local terms for mahogany when the marketplace allows retrieval. The buttons open searches in each marketplace. A phone may open an installed marketplace app when it supports these links.':'Przy braku odczytanych ofert automatyczna aktualizacja próbuje także określeń mahoniu w języku portalu, o ile portal pozwala na odczyt. Wyszukiwanie mahoniowych łodzi można też otworzyć osobnym przyciskiem. Przyciski otwierają wyszukiwanie w wybranym serwisie. Telefon może otworzyć jego zainstalowaną aplikację, jeśli obsługuje te linki; w pozostałych przypadkach otworzy stronę portalu.';
  panel.innerHTML=`<section class="portal-more" aria-label="${EN?'Marketplace listing availability':'Dostępność ofert w portalach'}"><h3>${EN?'Listings directly on marketplaces':'Oferty bezpośrednio w portalach'}</h3><p>${intro1}</p><p>${intro2}</p><div class="market-source-grid">${groups.map(group=>{
    const p=group[0],health=window.PSKL_MARKET.sourceStatus(p.name,status);
    return `<article class="market-source" data-source="${esc(p.id)}" data-state="${health.state}"><h4>${esc(p.name)}</h4><p class="market-source-scope">${EN?'Market':'Rynek'}: ${esc(EN?countryEN(p.country):p.country)}${p.foreign?` · ${EN?'outside selected country':'poza wybranym krajem'} (${esc(EN?countryEN(c.country):c.country)})`:''}</p><strong class="market-source-state">${label[health.state]}</strong><p>${esc(EN?healthEN[health.state]:health.text)}</p><a class="button primary" href="${esc(p.href)}" target="_blank" rel="noopener">${EN?'Open':'Otwórz'} ${esc(p.name)} ↗</a><small>${EN?'Query':'Zapytanie'} (${esc(EN?languageEN(p.language):p.language)}): ${esc(p.query||(EN?'all boats':'wszystkie łodzie'))}</small>${group.filter(v=>!v.mahogany).length>1?`<div class="market-source-variants"><span>${EN?'Also check:':'Sprawdź też:'}</span>${group.filter(v=>!v.mahogany).slice(1).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}${group.some(v=>v.mahogany)?`<div class="market-source-variants"><span>${EN?'No results? Search for mahogany boats:':'Brak wyników? Szukaj mahoniowych:'}</span>${group.filter(v=>v.mahogany).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}</article>`;
  }).join('')}</div><p class="market-status">${EN?(timestamp?`Status from the latest update attempt: ${esc(timestamp)}.`:status?.failed?'The index update is unavailable.':'Checking the index update.'):(timestamp?`Status według ostatniej próby aktualizacji: ${esc(timestamp)}.`:status?.failed?'Aktualizacja bazy jest niedostępna.':'Trwa sprawdzanie aktualizacji bazy.')} ${EN?'PSKL results come from a partial index. Confirm hull material and availability in the original listing.':'Wyniki w PSKL pochodzą z częściowej bazy. Materiał kadłuba i dostępność potwierdź w ogłoszeniu.'}${c.country?(EN?` Selected country: ${esc(countryEN(c.country))}. Foreign buttons show listings outside this country. Select location directly on international marketplaces.`:` Wybrany kraj: ${esc(c.country)}. Przyciski zagraniczne pokazują oferty poza tym krajem. W portalach międzynarodowych lokalizację wybiera się bezpośrednio w serwisie.`):''}</p></section>`;
}
