...
function renderMarketCoverage(){
  ...
  panel.innerHTML=`<section class="portal-more" aria-label="${EN?'Marketplace listing availability':'Dostępność ofert w portalach'}"><h3>${EN?'Listings directly on marketplaces':'Oferty bezpośrednio w portalach'}</h3><p>${intro1}</p><p>${intro2}</p><div class="market-source-grid">${groups.map(group=>{
    ...
    return `<article class="market-source" data-source="${esc(p.id)}" data-state="${health.state}"><h4>${esc(p.name)}</h4><p class="market-source-scope">${EN?'Market':'Rynek'}: ${esc(p.country)}${p.foreign?` · ${EN?'outside selected country':'poza wybranym krajem'} (${esc(c.country)})`:''}</p><strong class="market-source-state">${label[health.state]}</strong><p>${esc(EN?healthEN[health.state]:health.text)}</p><a class="button primary" href="${esc(p.href)}" target="_blank" rel="noopener">${EN?'Open':'Otwórz'} ${esc(p.name)} ↗</a><small>${EN?'Query':'Zapytanie'} (${esc(p.language)}): ${esc(p.query||(EN?'all boats':'wszystkie łodzie'))}</small>${group.filter(v=>!v.mahogany).length>1?`<div class="market-source-variants"><span>${EN?'Also check:':'Sprawdź też:'}</span>${group.filter(v=>!v.mahogany).slice(1).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}${group.some(v=>v.mahogany)?`<div class="market-source-variants"><span>${EN?'No results? Search for mahogany boats:':'Brak wyników? Szukaj mahoniowych:'}</span>${group.filter(v=>v.mahogany).map(v=>`<a href="${esc(v.href)}" target="_blank" rel="noopener">${esc(v.query)} ↗</a>`).join('')}</div>`:''}</article>`;
  }).join('')}</div><p class="market-status">${EN?(timestamp?`Status from the latest update attempt: ${esc(timestamp)}.`:status?.failed?'The index update is unavailable.':'Checking the index update.'):(timestamp?`Status według ostatniej próby aktualizacji: ${esc(timestamp)}.`:status?.failed?'Aktualizacja bazy jest niedostępna.':'Trwa sprawdzanie aktualizacji bazy.')} ${EN?'PSKŁ results come from a partial index. Confirm hull material and availability in the original listing.':'Wyniki w PSKŁ pochodzą z częściowej bazy. Materiał kadłuba i dostępność potwierdź w ogłoszeniu.'}${c.country?(EN?` Selected country: ${esc(c.country)}. Foreign buttons show listings outside this country. Select location directly on international marketplaces.`:` Wybrany kraj: ${esc(c.country)}. Przyciski zagraniczne pokazują oferty poza tym krajem. W portalach międzynarodowych lokalizację wybiera się bezpośrednio w serwisie.`):''}</p></section>`;
}
