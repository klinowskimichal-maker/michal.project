...
if(window.PSKL_PIRATE_SCENES_10S||!Element.prototype.animate)return;
...
const active=new Set(),pendingWaits=new Set(),poses=['look','crouch','laugh','kneel','dance','salute'];
...
function wait(ms){
 return new Promise((resolve,reject)=>{
  if(interrupted){reject(new Error('Scene paused'));return}
  const entry={timer:null,reject};
  entry.timer=setTimeout(()=>{pendingWaits.delete(entry);resolve()},ms);
...
function clear(){
 pendingWaits.forEach(w=>{clearTimeout(w.timer);w.reject(new Error('Scene paused'))});pendingWaits.clear();
 active.forEach(a=>a.cancel?.());active.clear();layer.replaceChildren();
 const a=assistant();if(a){a.style.opacity='';a.style.pointerEvents=''}
 const p=pirate();if(p)poses.forEach(x=>p.classList.remove(x));
}
function clonePirate(){const c=pirate()?.cloneNode(true);if(c){c.removeAttribute('aria-label');c.tabIndex=-1;c.inert=true;c.style.pointerEvents='none';poses.forEach(x=>c.classList.remove(x))}return c}
...
async function cannon(){
 ...
 await wait(1400);
 c.style.opacity='1';c.getAnimations().forEach(a=>{a.cancel();active.delete(a)});
 c.classList.remove('lit');c.classList.add('fire');
...
async function run(){
 if(running||document.hidden||motion?.matches||$('.pskl-chat.open')||$('#nav.open')||assistant()?.contains(document.activeElement))return;
 ...
 try{
  ...
  if(hidden&&!document.hidden&&!interrupted)assistant()?.animate([{opacity:0},{opacity:1}],{duration:350,easing:'ease-out',fill:'none'});
  index=(index+1)%scenes.length;running=false;
 }
}
function pause(){if(!running)return;interrupted=true;clear()}
document.addEventListener('visibilitychange',()=>{if(document.hidden)pause()});
motion?.addEventListener?.('change',e=>{if(e.matches)pause()});
// Stop a scene before navigation, typing or opening the adviser.
// Capture runs before the pirate's own click handler.
['pointerdown','focusin'].forEach(type=>document.addEventListener(type,pause,true));
