...
function patchLinks(){
  $$('.portal-result').forEach(a=>{
    ...
    if(name==='OLX')a.href=`https://www.olx.pl/sport-hobby/sporty-wodne/q-${slug(term)}/`;
    else if(name==='Allegro')a.href=`https://allegro.pl/kategoria/lodzie-motorowki-4084?oferta-dotyczy=sprzeda%C5%BC&string=${encodeURIComponent(term)}`;
    else if(name==='FINN.no')a.href=`https://www.finn.no/mobility/search/boat?query=${encodeURIComponent(term)}`;
    else if(name==='Blocket')a.href=`https://www.blocket.se/mobility/search/boat?q=${encodeURIComponent(term)}`;
    const em=a.querySelector('em');if(em)em.textContent=term;
    const cta=a.querySelector('strong:last-child');if(cta)cta.textContent='Otwórz pełne aktualne wyniki ↗';
  });
